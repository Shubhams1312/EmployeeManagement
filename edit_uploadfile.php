...
$qry = "SELECT * FROM `tbl_image` WHERE active ='1' AND emp_id = '".$url[1]."'";
	$query =  "SELECT * FROM `tbl_profile` WHERE active ='1' AND emp_id = '".$url[1]."'";
	...
	if($result_arr['name'] != ""){
		$imagePath = 'http://localhost/hr_management/images/document/profile/'.$result_arr['name']; 
	}else{
		$imagePath = "upload profile";
								...
								<input type="hidden" id="emp_id" value="<?php echo $url[1]; ?>">						
									<div class="form-group">
										...
										<div class="col-sm-2">
											...
											<button type="button" class="btn btn-danger update" id=" " >Update</button>
			...
			$('.upload').click(function() {   
				...
				$.ajax({
					...
					success: function(response) {
						...
						// 	image.alt = "Uploaded Profile Picture";
